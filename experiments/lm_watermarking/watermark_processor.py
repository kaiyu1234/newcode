...
class WatermarkBase:
    ...
    def _get_quantile(self, input_ids: torch.LongTensor,cur_token):
        raise NotImplementedError #should not be used again, yield inferior results
        self._seed_rng(input_ids)

        greenlist_size = int(self.vocab_size * self.gamma)
        vocab_permutation = torch.randperm(self.vocab_size, device=input_ids.device, generator=self.rng)
        assert self.select_green_tokens # directly
        
        # greenlist_ids = vocab_permutation[:greenlist_size] # new
        
        # mid_res=torch.where(vocab_permutation==cur_token)
        # print('len(mid_res)',len(mid_res))
        # print('mid_res[0].shape:',mid_res[0].shape)
        # print('self.vocab_size:',self.vocab_size)
        
        # raise NotImplementedError
        quantile=(torch.where(vocab_permutation==cur_token)[0]+1)/self.vocab_size
        return quantile

    # def _get_in_green(self, input_ids: torch.LongTensor,cur_token):
    #     green_list_ids=self._get_greenlist_ids(input_ids)

    #     assert self.select_green_tokens # directly
    #     return cur_token in green_list_ids
