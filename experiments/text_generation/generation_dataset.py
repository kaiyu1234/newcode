...
def get_wb_2_1():
    prompts = []
    with open("./data/2-1_longform_qa.jsonl", "r") as f:
        lines = f.readlines()
    for line in lines:
        if len(line) < 3:
            continue
        ...
        prompts.append(
            f"You are a helpful assistant, please answer the following question within 300 words:\n{cur_context}\n{cur_input}"
        )
    return prompts
...
def get_wb_2_2():
    prompts = []
    with open("./data/2-2_finance_qa.jsonl", "r") as f:
        lines = f.readlines()
    for line in lines:
        if len(line) < 3:
            continue
        ...
        cur_context, cur_input = input_dict["context"], input_dict["input"]
        prompts.append(
            f"You are a helpful assistant, please answer the following question with financial knowledge within 300 words:\n{cur_context}\n{cur_input}"
        )
    return prompts
